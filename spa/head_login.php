...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if(empty(trim($_POST["user_name"])))
    {
         ...
         echo'alert("Please enter a Username."); location.href="head_login.php"';
         echo '</script>';
    ...
    if(empty(trim($_POST["password"])))
    {
         ...
         echo '<script language="javascript">';
         echo'alert("Please enter your Password."); location.href="head_login.php"';
         echo '</script>';
...
{
    $sql = "SELECT status FROM head_login WHERE user_name = '$uname'";
...
{
    echo '<script language="javascript">';
    echo'alert("Please Activate Your Account."); location.href="head_login.php"';
    echo '</script>';
...
else
{
    ...
    if(empty($uname_err) && empty($password_err))
    {
        ...
        $sql = "SELECT id, user_name, password, department, dob FROM head_login WHERE user_name = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql))
        {
            ...
            if(mysqli_stmt_execute($stmt))
            {
                ...
                {                    
                    ...
                    mysqli_stmt_bind_result($stmt, $id, $uname, $hashed_password, $department, $dob);
                        ...
                        if(password_verify($password, $hashed_password))
                         {
                            ...
                            //header("location: head_twostep.php");

                            $otp = rand(11111,99999);
                            $query = "update head_login set otp = '$otp' where user_name = '$uname'";
                            mysqli_query($link, $query);

                            $sql = "SELECT email FROM head_login WHERE user_name = '$uname'";
                            $result =  $link->query($sql);
                            if($result -> num_rows > 0)
                            {
                                while($row = $result -> fetch_assoc())
                          ...
                          else
                          {
                           // Password is not valid, display a generic error message
                            echo '<script language="javascript">';
         					echo'alert("Invalid Admin Username or Password."); location.href="head_login.php"';
         					echo '</script>';
			...
			<div class="panel panel-default" style="border-top:2px solid #087ec2;">
				<div class="panel-heading"> 
					...
					<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
						<!-- admin login image -->
						  	<img src="images/departmentlogin.png" class="img-circle img-responsive center-block d-block mx-auto" id="login"/>
							<br><br>
