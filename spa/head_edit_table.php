...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	if(empty(trim($_POST["staff_name"])))
	{
		...
		echo'alert("Please enter a staff Name."); location.href="head_edit.php"';
		echo '</script>';
	}
	else
	{
	...
	if(!preg_match("/^[a-zA-Z-'. ]*$/",$tsname))
	{
		$error++;
		echo '<script language="javascript">';
		echo'alert("Only letters, white space and dots are allowed."); location.href="head_edit.php"';
		echo '</script>';
	} 
	else
	{
	...
	if (empty(trim($_POST["email"]))) 
	{
		$error++;
		echo '<script language="javascript">';
		echo'alert("Please enter a Email."); location.href="head_edit.php"';
		echo '</script>';
	} 
	else 
	{
		...
		if (!filter_var($temail, FILTER_VALIDATE_EMAIL)) 
		{
			$error++;
			echo '<script language="javascript">';
		  echo'alert("Invalid email format."); location.href="head_edit.php"';
			echo '</script>';
		}
		else
		{
	...
	if (empty(trim($_POST["dob"])))
	{
		$error++;
		echo '<script language="javascript">';
		echo'alert("Please Enter the Date of Birth."); location.href="head_edit.php"';
		echo '</script>';
	...
	if($error==0)
	{
		$query = "UPDATE head_login SET staff_name = '$aname', email = '$email', dob = '$dob' WHERE user_name = '$uname'";
		$query_run = mysqli_query($link,$query);

		if($query_run)
		{
			echo '<script language="javascript">';
			echo'alert("Update Successfully"); location.href="head_edit.php"';
			echo '</script>';
		}
	}
	else
	{
		echo '<script language="javascript">';
		echo'alert("Not Updated"); location.href="head_edit.php"';
		echo '</script>';
	}
}
