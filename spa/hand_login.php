...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if(empty(trim($_POST["user_name"])))
    {
       ...
       echo'alert("Please enter a Username."); location.href="hand_login.php"';
       echo '</script>';
...
if(empty(trim($_POST["password"])))
{
   ...
   echo '<script language="javascript">';
   echo'alert("Please enter your Password."); location.href="hand_login.php"';
   echo '</script>';
...
{
    $sql = "SELECT status FROM hand_login WHERE user_name = '$uname'";
...
{
    echo '<script language="javascript">';
    echo'alert("Please Activate Your Account."); location.href="hand_login.php"';
    echo '</script>';
...
else
{
    ...
    if(empty($uname_err) && empty($password_err))
    {
        ...
        $sql = "SELECT id, user_name, staff_name, password, dob FROM hand_login WHERE user_name = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql))
        {
            ...
            if(mysqli_stmt_execute($stmt))
            {
                ...
                {                    
                    ...
                    mysqli_stmt_bind_result($stmt, $id, $uname, $sname, $hashed_password, $adob);
                    if(mysqli_stmt_fetch($stmt))
                    {
                        if(password_verify($password, $hashed_password))
                        {
                            ...
                            $query = "update hand_login set otp = '$otp' where user_name = '$uname'";
                            mysqli_query($link, $query);

                            $sql = "SELECT email FROM hand_login WHERE user_name = '$uname'";
                            $result =  $link->query($sql);
                            if($result -> num_rows > 0)
                            {
                                while($row = $result -> fetch_assoc())
                            ...
                            if(!$mail->send()) 
                            {
                                ...
                                header("location:hand_twostep.php");
                            }  
                            else 
                            {
                                header("location:hand_twostep.php");
                        ...
                        else
                        {
                           // Password is not valid, display a generic error message
                            echo '<script language="javascript">';
                            echo'alert("Invalid Admin Username or Password."); location.href="hand_login.php"';
                            echo '</script>';
...
<div class="panel-heading"> 
 <h1 class="panel-title" style="color:#087ec2; font-weight:bold;"><span class="glyphicon glyphicon-lock"></span> Hand Login</h1>
</div>
<div class="panel-body">
 <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
  <!-- admin login image -->
  <img src="images/stafflogin.png" class="img-circle img-responsive center-block d-block mx-auto" id="login"/>
  <br><br>
...
<div class="panel-heading">
<h1 class="panel-title" style=" color:#087ec2; font-weight: bold; text-align: center;">Hand Login</h1>
</div>
</div>
</div>
</div>
