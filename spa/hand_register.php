...
function sendemail_verify($uname, $verify_token, $email)
{
	...
	<h5>Verify your email address to login with the below link</h5><br><a href = 'http://localhost/spa/hand_verify_email.php?UserName=$uname&Token=$verify_token'>Click Me</a>";
...
$uname = $password = $cpassword = $sname = $email = $tsname = $temail= $dob = $verify_token = "";
$uname_err = $password_err = $cpassword_err = $sname_err = $email_err = $dob_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	// Prepare a select statement
		$sql = "SELECT id FROM hand_login WHERE user_name = ?";
	...
	{
		$sname_err = "Only letters, white space and dots are allowed.";
	} 
	...
	else 
	{
		...
		{
			$sql = "SELECT id FROM hand_login WHERE email = ?";
  ...
  // Check input errors before inserting in database
	if(empty($uname_err) && empty($password_err) && empty($cpassword_err) && empty($aname_err) && empty($email_err) && empty($dob_err))
	{
		...
		$sql = "INSERT INTO hand_login (user_name, staff_name, password, email, dob, verify_token) VALUES (?, ?, ?, ?, ?, ?)";
		
		if($stmt = mysqli_prepare($link, $sql))
		{
            // Bind variables to the prepared statement as parameters
			mysqli_stmt_bind_param($stmt, "ssssss", $param_username, $param_sname ,$param_password, $param_email, $param_dob, $param_verify_token);
            ...
            if(mysqli_stmt_execute($stmt))
            {
            	...
            	echo'alert("Successfully Registered."); location.href="hand_register.php"';
            	echo '</script>';
            } 
            else
            {
					...
					<form  method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
						<div class="card-body">
							...
							<ul class="nav nav-tabs">
								...
								<li class="nav-item">
									<a class="nav-link active" href="hand_register.php" id="navtabs" >Hand Registration</a>
								</li>
								<li class="nav-item">
							...
							<div class="container-fluid"><br>
								<div class="row">
									<div class="col-sm-6 col-sm-6">
										<div class="card" style="border-top:2px solid #087ec2;">
											<div class="card-header bg-light" style="height: 45px;">
												<h6 class="card-title card-text" style="color:#087ec2; font-weight:bold; "><i class="fa fa-lock"></i> Hand - Login Details</h6>
											</div>
									...
									<div class="col-sm-6 col-sm-6">
										<div class="card" style="border-top:2px solid #087ec2;">
											<div class="card-header bg-light" style="height: 45px;">
												<h6 class="card-title card-text" style="color:#087ec2; font-weight:bold;"><i class="fa fa-user"></i> Hand - Personal Details</h6>
											</div>
											<div class="card-body" style="height: 350px">
												...
												<div class="form-group">
													...
													<input type="text" name="staff_name" class="form-control <?php echo (!empty($sname_err)) ? 'is-invalid' : ''; ?> " value="<?php echo $sname; ?>" id="sname" placeholder="Enter Faculty Name">
