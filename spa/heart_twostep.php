...
if(!isset($_SESSION["steponeverification"]) || $_SESSION["steponeverification"] !== true){
	header("location: heart_login.php");
	exit;
}
...
$sql = "SELECT otp,email FROM heart_login WHERE user_name = '$uname'";
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	if(empty(trim($_POST["user_dob"])))
	{
        ...
        echo'alert("Please enter a Date of Birth."); location.href="heart_twostep.php"';
        echo '</script>';
        $err_dob = "yes";
	...
	{
		echo '<script language="javascript">';
        echo'alert("Invalid Date of Birth."); location.href="heart_twostep.php"';
        echo '</script>';
        $err_dob = "yes";		
	}
	else
	...
	{
		echo '<script language="javascript">';
        echo'alert("Please enter a OTP."); location.href="heart_twostep.php"';
        echo '</script>';
        $err_otp = "yes";
	}
	...
	{
		echo '<script language="javascript">';
        echo'alert("Invalid OTP."); location.href="heart_twostep.php"';
        echo '</script>';
        $err_otp = "yes";		
	}
	else
 	...
 	if(empty($err_dob) && empty($err_otp))
    {
    	session_start();
     	...
     	header("location: heart_home.php");
    }
}
				...
				<div class="panel-heading"> 
					...
					<!-- Date of Birth -->
												...
												<div class="form-group">
													...
													<input type="text" name="user_otp" class="form-control <?php //echo (!empty($dob_err)) ? 'is-invalid' : ''; ?> " value="<?php //echo $dob; ?>" id="dob" placeholder = "Enter OTP.">
