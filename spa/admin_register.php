...
function sendemail_verify($uname, $verify_token, $email)
{
	...
	<h5>Verify your email address to login with the below link</h5><br><a href = 'http://localhost/spa/admin_verify_email.php?UserName=$uname&Token=$verify_token'>Click Me</a>";
...
$uname = $password = $cpassword = $aname = $email = $taname = $temail= $dob = $verify_token = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
  ...
  // Check input errors before inserting in database
	if(empty($uname_err) && empty($password_err) && empty($cpassword_err) && empty($aname_err) && empty($email_err) && empty($dob_err))
	{
		...
		$sql = "INSERT INTO admin_login (user_name, admin_name, password, email, dob, verify_token) VALUES (?, ?, ?, ?, ?, ?)";
		
		if($stmt = mysqli_prepare($link, $sql))
		{
            // Bind variables to the prepared statement as parameters
			mysqli_stmt_bind_param($stmt, "ssssss", $param_username, $param_aname ,$param_password, $param_email, $param_dob, $param_verify_token);
